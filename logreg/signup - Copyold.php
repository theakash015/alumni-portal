...
$email = $password = $confirm_password = $fname = $lname = $phone = $gender = $role = "";

$email_err = $password_err = $confirm_password_err = $fname_err = $lname_err = $phone_err = $gender_err = $role_err = "";
...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (empty($email_err) && empty($password_err) && empty($confirm_password_err) && empty($fname_err) && empty($lname_err) && empty($phone_err) && empty($gender_err) && empty($role_err)) {

        $sql = "INSERT INTO `users` (`first name`, `last name`, `email`, `gender`, `password`, `phoneno`, `role`) VALUES (?, ?, ?, ?, ?, ?, ?)";

        $stmt = mysqli_prepare($conn, $sql);
        if ($stmt) {

            mysqli_stmt_bind_param($stmt, "sssssss", $param_fname, $param_lname, $param_email, $param_gender, $param_password, $param_phone, $param_role);
            ...
            <div class="card">
                ...
                <form class="form needs-validation" action="./signup.php" method="POST" novalidate>
                    ...
                    <?php if (isset($_SESSION["gimg"])) { ?>
                        <div class="form-group row mt-3">
                            <div class="form-group col-md-12 text-center">
                                <?php $gimg = $_SESSION["gimg"] ?>
                                <img src="<?php echo $gimg; ?>" class="rounded mx-auto" />
                            </div>
                        </div>
                    <?php } ?>
                    ...
                    <div class="phppool">
                        ...
                        $arr_err = array("$email_err", "$password_err", "$confirm_password_err", "$fname_err", "$lname_err", "$phone_err");
