...
if (isset($_POST['send'])) {
    ...
    if (isset($_FILES['file'])) {
        ...
        if (in_array($fileActualExt, $allowed)) {
            if ($fileError === 0) {
                if ($fileSize < 1000000) {
                    $fileDestination = '../uploads/eventdetails/' . $fileName;
    ...
    $q = "INSERT INTO `events` (`title`, `description`, `agenda`, `note`, `date`, `starttime`, `endtime`, `PDF`, `location`) VALUES ('$title', '$description', '$agenda', '$note', '$date', '$start', '$end', '$fileDestination', '$location')";

    $run = mysqli_prepare($conn, $q);

    if (mysqli_stmt_execute($run)) {

        $newtitle = "New event added - " . $title;
        $q2 = "INSERT INTO `noticeandupdates` (`title`, `description`, `link`) VALUES ('$newtitle', 'event', 2)";
...
<body>
    ...
    <div class="container mt-4">
        ...
        <div class="card">
            <form class="form needs-validation" action="./addeve.php" method="POST" enctype="multipart/form-data" novalidate>
                <?php
                if (isset($_POST['send'])) {
                    if (mysqli_stmt_execute($run)) {
                ?>
                        <div class="alert alert-success alert-dismissible fade show" role="alert">
                            <strong class="msg"><?php echo "Event added sucessfully"; ?></strong>
                ...
                <h3 class="bl">Add Events</h3>
                <div class="form-group row mt-3">
                    <div class="form-group col-md-12">
                        <input type="text" class="form-control" name="evetitle" id="evetitle" placeholder="Event Title" required />
                    </div>
                </div>

                <div class="form-group row mt-3">
                    <div class="form-group col-md-12">
                        <textarea name="evedescription" id="evedescription" class="form-control" rows="5" required placeholder="Event Description"></textarea>
                    </div>
                </div>

                <div class="form-group row mt-3">
                    <div class="form-group col-md-12">
                        <textarea name="eveagenda" class="form-control" rows="2" placeholder="Event Agenda"></textarea>
                    </div>
                </div>

                <div class="form-group row mt-3">
                    <div class="form-group col-md-12">
                        <textarea name="evenote" class="form-control" rows="2" placeholder="Event Note"></textarea>
                    </div>
                </div>
                ...
                <div class="form-group row mt-3">
                    <div class="form-group col-md-7">
                        <input type="date" class="form-control" name="evedate" placeholder="dd-mm-yyyy" min="<?php echo date("Y-m-d"); ?>" max="2030-12-31" required />
                    </div>
                    <div class="form-group col-md-5">
                        <input type="text" class="form-control" name="evelocation" id="evelocation" placeholder="Event Location" required />
                    </div>
                </div>

                <div class="form-group row mt-3">
                    <div class="form-group col-md-6">
                        <label for="evetimestart">Event starting time</label>
                        <input type="time" class="form-control" id="evetimestart" name="evetimestart" required />
                    </div>
                    <div class="form-group col-md-6">
                        <label for="evetimeend">Event ending time</label>
                        <input type="time" class="form-control" id="evetimeend" name="evetimeend" required />
                    </div>
                </div>

                <div class="form-group row mt-3">
                    <div class="form-group col-md-12">
                        <label for="file">Event details - PDF / Image</label>
